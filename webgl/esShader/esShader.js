...
function esLoadShader ( type, shaderSrc )
{
   ...
   shader = gl.createShader ( type ); // Tutorial: glCreateShader(GL_VERTEX_SHADER);

   if ( shader == 0 )
   	return 0;

   // Load the shader source
   gl.shaderSource ( shader, shaderSrc ); // Tutorial: glShaderSource(vertexShader, 1, &vertexShaderSource, null);

   // Compile the shader
   gl.compileShader ( shader ); // Tutorial: glCompileShader(vertexShader);
